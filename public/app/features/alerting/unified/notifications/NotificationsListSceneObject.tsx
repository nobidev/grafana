...
function AlertEnrichments({ enrichments }: AlertEnrichmentsProps) {
  ...
  return (
    <Stack direction="column" gap={1}>
      <Text variant="bodySmall" color="secondary">
        <strong>
          <Trans i18nKey="alerting.notifications-scene.enrichments">Enrichments:</Trans>
        </strong>
      </Text>
        ...
        <Stack direction="column" gap={0.5}>
          <Stack direction="row" gap={1} alignItems="center">
            ...
            {item.exploreLink && (
              <LinkButton size="sm" variant="secondary" icon="compass" href={item.exploreLink} target="_blank">
                <Trans i18nKey="alerting.notifications-scene.enrichment-explore">View in Explore</Trans>
              </LinkButton>
            )}
          </Stack>
          ...
          {(!item.lines || item.lines.length === 0) && (
            <Text variant="bodySmall" color="secondary" italic>
              <Trans i18nKey="alerting.notifications-scene.enrichment-no-logs">No log lines</Trans>
            </Text>
          )}
        </Stack>
      </div>
