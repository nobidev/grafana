...
describe('getPluginExtensionRegistries', () => {
  beforeEach(() => {
    ...
    mockLogger('grafana.utils.getCachedPromise');
  });
  ...
  test('should not cache promise if getAppPluginMetas throws', async () => {
    ...
    expect(second).toBe(third);
    expect(getLogger('grafana.utils.getCachedPromise').logError).toHaveBeenCalledTimes(1);
    expect(getLogger('grafana.utils.getCachedPromise').logError).toHaveBeenCalledWith(
