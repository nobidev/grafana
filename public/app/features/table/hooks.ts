...
// Keep row operations and column organization in one ordered stage, regardless of interaction order.
export const TABLE_TRANSFORMATIONS_TAG = 'table:view';
...
export function useAdHocColumnState(frames: DataFrame[], frameIndex: number, enabled: boolean) {
  const adHoc = useAdHocTransformations(TABLE_TRANSFORMATIONS_TAG);
