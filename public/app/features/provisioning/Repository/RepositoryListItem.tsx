          ...
          {status?.stats?.length && (
            <Stack gap={1} direction="row" wrap>
              {status.stats.map((stat, index) => {
                ...
                const href = descriptor.getListUrl(repository);
                const label = descriptor.getCountLabel(stat.count);

                // Unknown kinds have no destination — show a non-interactive badge.
                return href ? (
                  <LinkButton
                    key={index}
                    fill="outline"
                  ...
                  <Button key={index} fill="outline" size="md" variant="secondary" icon={descriptor.icon} disabled>
                    {label}
                  </Button>
                );
              })}
