...
export const getSecretNameAsHexString = async (fieldName: string): Promise<string> => {
  const digestBuffer = await getSecretDigest(fieldName).then((value) => {
    return value;
  });
    ...
    if (dataSource.secureJsonData) {
      ...
      for (let [k, v] of Object.entries(dataSource.secureJsonData)) {
        if (v === '') {
          let value = {
            remove: true,
            name: await getSecretNameAsHexString(dataSource.uid + '|' + k).then((value) => {
