...
it.each(['hide-first', 'filter-first'])(
  ...
  async (order) => {
    ...
    try {
      ...
      expect(api.get(TABLE_TRANSFORMATIONS_TAG).map((config) => config.id)).toEqual(['tableView', 'organize']);
      ...
      await waitFor(() => expect(transformer.state.data?.series[0].fields[0].values).toEqual(['one']));
      expect(api.get(TABLE_TRANSFORMATIONS_TAG).map((config) => config.id)).toEqual(['tableView', 'organize']);
