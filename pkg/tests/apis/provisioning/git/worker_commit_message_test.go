...
// each commit-producing worker (delete, move, fixfoldermetadata, export,
// migrate). They guard the wiring added in grafana/git-ui-sync-project#1164.
...
func TestIntegrationGit_DeleteJob_CommitMessage(t *testing.T) {
	...
	require.Equal(t, expectedMessage, latestCommitSubject(t, local))
...
func TestIntegrationGit_FixFolderMetadataJob_CommitMessage(t *testing.T) {
	helper := sharedGitHelper(t)

	const (
		repoName = "fix-folder-metadata-commit-msg"
		dashUID  = "fix-folder-metadata-dash"
		dashFn   = "nested/dashboard.json"
	)

	_, local := helper.CreateGitRepo(t, repoName, map[string][]byte{
		dashFn: common.DashboardJSON(dashUID, "Dashboard in nested folder", 1),
	})
	helper.SyncAndWait(t, repoName)

	const expectedMessage = "Fix folder metadata via JobSpec"
	helper.TriggerJobAndWaitForComplete(t, repoName, provisioning.JobSpec{
