...
func testDataStoreGetResourceStatsWithLimit(t *testing.T, ctx context.Context, ds *dataStore) {
	...
	// The effective limit never drops below minCount+1: with minCount=5 and
	// countLimit=1, ns-big (8 live) is still counted enough to pass the filter,
	// while ns-small (3 live) is correctly excluded.
	floored, err := ds.GetResourceStatsWithLimit(ctx, NamespacedResource{}, 5, 1)
	require.NoError(t, err)
	gotFloored := byNamespace(floored)
	require.GreaterOrEqual(t, gotFloored["ns-big"], int64(6))
	require.NotContains(t, gotFloored, "ns-small", "namespace at or below minCount must be excluded")
