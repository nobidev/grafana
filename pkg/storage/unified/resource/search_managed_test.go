...
func TestGetStatsUsesDiscovery(t *testing.T) {
	const ns = "ns"
	grA := NamespacedResource{Namespace: ns, Group: "group", Resource: "dashboards"}
	grB := NamespacedResource{Namespace: ns, Group: "group", Resource: "folders"}

	storage := &mockStorageBackend{
		// The counts here are deliberately wrong: GetStats must count via the
		// index, not reuse these stat counts.
		resourceStats: []ResourceStats{
			{NamespacedResource: grA, Count: 999},
			{NamespacedResource: grB, Count: 999},
		},
	}
	search := &mockSearchBackend{
		cache: map[NamespacedResource]ResourceIndex{
			grA: &MockResourceIndex{docCount: 3},
			grB: &MockResourceIndex{docCount: 7},
		},
	}
	server := newDiscoveryTestSearchServer(t, storage, search)

	rsp, err := server.GetStats(t.Context(), &resourcepb.ResourceStatsRequest{Namespace: ns})
	require.NoError(t, err)
	require.Nil(t, rsp.Error)

	got := map[string]int64{}
	for _, s := range rsp.Stats {
		got[s.Group+"/"+s.Resource] = s.Count
	}
	require.Equal(t, map[string]int64{"group/dashboards": 3, "group/folders": 7}, got)
