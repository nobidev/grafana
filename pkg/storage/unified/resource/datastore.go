...
func (d *dataStore) processGroupResourceStats(ctx context.Context, gr GroupResource, namespace string, minCount, countLimit int) ([]ResourceStats, error) {
	...
	// Never stop below the minCount filter, otherwise a capped namespace could be
	// dropped even though its exact count would pass it.
	limit := countLimit
	if limit > 0 && limit <= minCount {
		limit = minCount + 1
	}
	...
	// Iterate in descending key order so the first key seen for each
	// group/resource/namespace/name is its latest version. That alone decides
	// liveness (count unless the latest event is a delete), so a resource's older
	// versions can be skipped and no per-version state is needed. Keys are ordered
	// group/resource/namespace/name/rv, so one namespace finishes before the next.
	// open distinguishes "no partition yet" from a cluster-scoped resource, whose
	...
	emit := func() {
		if open && liveCount > int64(minCount) {
			rv := maxRV
			if countLimit > 0 {
				rv = 0 // count may be capped, so this is not necessarily the newest version
			}
	...
	for {
		...
		for key, err := range d.kv.Keys(ctx, dataSection, ListOptions{StartKey: basePrefix, EndKey: endKey, Sort: SortOrderDesc}) {
			...
			if limit > 0 && liveCount >= int64(limit) {
				ns := curNS
				emit()
