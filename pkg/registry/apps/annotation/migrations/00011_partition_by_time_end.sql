-- +goose Up
-- List filters on overlap (time <= to AND time_end >= from), and only a bound on the
-- partition key prunes. to is usually ~now, so partitioning on time pruned nothing;
-- partitioning on time_end prunes every partition that ended before the window.
...
BEGIN
    -- Moving the old tables to another schema takes their indexes and constraints
    -- with them, freeing every name for the new table.
    ...
    -- Names and bounds must match getPartitionName and getPartitionBounds.
    FOR week_start IN
