...
func (s *PostgreSQLStore) Update(ctx context.Context, anno *annotationV0.Annotation) (*annotationV0.Annotation, error) {
	// legacy_data is only overwritten when present on the incoming object; a nil
	// value leaves the stored blob untouched, mirroring the legacy API's behaviour
	// of only updating `data` when the client explicitly sends it.
	var legacyData *string
	if d := getLegacyData(anno); d != "" {
		legacyData = &d
	}
	...
	query := `
		UPDATE annotations
		SET text = $1, tags = $2, scopes = $3, legacy_data = COALESCE($4, legacy_data)
