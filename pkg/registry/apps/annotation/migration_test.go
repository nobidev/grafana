...
func TestIntegrationMigrations(t *testing.T) {
	...
	t.Run("00011_partition_by_time_end", func(t *testing.T) {
		...
		// Up to 00010 the table is partitioned on time.
		runMigrationsUpTo(t, ctx, pool, 10)

		const namespace = "stacks-migration-test"
		base := time.Date(2026, 9, 2, 12, 0, 0, 0, time.UTC) // a Wednesday
		...
		// The store's on-demand partitions must abut the migrated ones exactly, or
		// Postgres rejects them as overlapping.
