...
func TestApplyIncrementalChanges_FolderMetadataDiff(t *testing.T) {
	t.Run("applies synthetic folder update before child updates", func(t *testing.T) {
		...
		affectedFolders, err := applyIncrementalChanges(ctx, diff, existingFoldersByPath, nil, repoResources, progress, tracer, span, newPermissiveMockQuotaTracker(t))
		require.NoError(t, err)
	...
	t.Run("deletes replaced folder after applying synthetic updates", func(t *testing.T) {
		...
		affectedFolders, err := applyIncrementalChanges(ctx, diff, existingFoldersByPath, replacedFolders, repoResources, progress, tracer, span, newPermissiveMockQuotaTracker(t))
		require.NoError(t, err)
		require.Empty(t, affectedFolders)
	})
}
