...
func TestFindDuplicateFolderUIDs(t *testing.T) {
	ctx := context.Background()

	t.Run("no _folder.json files returns nil", func(t *testing.T) {
		...
		result, err := FindDuplicateFolderUIDs(ctx, reader, source, "")
		require.NoError(t, err)
		require.Nil(t, result)
	})

	t.Run("all unique UIDs returns nil", func(t *testing.T) {
		reader := repository.NewMockReader(t)

		folderA := NewFolderManifest("uid-a", "Folder A")
		dataA, _ := json.Marshal(folderA)
		folderB := NewFolderManifest("uid-b", "Folder B")
		dataB, _ := json.Marshal(folderB)

		reader.On("Read", mock.Anything, "folderA/_folder.json", "").
			Return(&repository.FileInfo{Data: dataA}, nil)
		reader.On("Read", mock.Anything, "folderB/_folder.json", "").
			Return(&repository.FileInfo{Data: dataB}, nil)

		source := []repository.FileTreeEntry{
			{Path: "folderA", Blob: false},
			{Path: "folderA/_folder.json", Blob: true},
			{Path: "folderB", Blob: false},
			{Path: "folderB/_folder.json", Blob: true},
		}
	...
	t.Run("one duplicate pair", func(t *testing.T) {
		reader := repository.NewMockReader(t)

		folderA := NewFolderManifest("same-uid", "Folder A")
		dataA, _ := json.Marshal(folderA)
		folderB := NewFolderManifest("same-uid", "Folder B")
		dataB, _ := json.Marshal(folderB)
		...
		require.Equal(t, "same-uid", result[0].UID)
		require.Equal(t, "folderA/", result[0].FirstPath)
		require.Equal(t, "folderB/", result[0].OtherPath)
	})

	t.Run("same UID in three folders returns two entries", func(t *testing.T) {
		reader := repository.NewMockReader(t)

		for _, name := range []string{"a", "b", "c"} {
			folder := NewFolderManifest("shared-uid", "Folder "+name)
			data, _ := json.Marshal(folder)
			reader.On("Read", mock.Anything, name+"/_folder.json", "").
				Return(&repository.FileInfo{Data: data}, nil)
		}

		source := []repository.FileTreeEntry{
			{Path: "a/_folder.json", Blob: true},
			{Path: "b/_folder.json", Blob: true},
			{Path: "c/_folder.json", Blob: true},
		}

		result, err := FindDuplicateFolderUIDs(ctx, reader, source, "")
		require.NoError(t, err)
		require.Len(t, result, 2)
		require.Equal(t, "a/", result[0].FirstPath)
		require.Equal(t, "b/", result[0].OtherPath)
		require.Equal(t, "a/", result[1].FirstPath)
		require.Equal(t, "c/", result[1].OtherPath)
	})

	t.Run("ReadFolderMetadata error is propagated", func(t *testing.T) {
		reader := repository.NewMockReader(t)

		reader.On("Read", mock.Anything, "broken/_folder.json", "").
			Return(nil, fmt.Errorf("permission denied"))

		source := []repository.FileTreeEntry{
			{Path: "broken/_folder.json", Blob: true},
		}
...
func TestValidateFolderMetadata(t *testing.T) {
	...
	t.Run("no _folder.json files returns nil", func(t *testing.T) {
		...
		issues, err := ValidateFolderMetadata(ctx, reader, source, "")
		require.NoError(t, err)
	...
	t.Run("all valid returns nil", func(t *testing.T) {
		...
		folderA := NewFolderManifest("uid-a", "folderA")
		dataA, _ := json.Marshal(folderA)
		folderB := NewFolderManifest("uid-b", "folderB")
		dataB, _ := json.Marshal(folderB)
	...
	t.Run("duplicate UIDs detected", func(t *testing.T) {
		reader := repository.NewMockReader(t)

		folderA := NewFolderManifest("same-uid", "folderA")
		dataA, _ := json.Marshal(folderA)
		folderB := NewFolderManifest("same-uid", "folderB")
		dataB, _ := json.Marshal(folderB)
		...
		assert.Equal(t, FolderMetadataIssueDuplicateUID, issues[0].Type)
		assert.Equal(t, "folderB/", issues[0].Path)
		assert.Contains(t, issues[0].Message, "same-uid")
	})

	t.Run("multiple folders with duplicate UID", func(t *testing.T) {
		reader := repository.NewMockReader(t)

		// folderA has a unique UID
		folderA := NewFolderManifest("uid-a", "folderA")
		dataA, _ := json.Marshal(folderA)

		// folderB and folderC share the same UID
		dup1 := NewFolderManifest("dup-uid", "folderB")
		dup1Data, _ := json.Marshal(dup1)
		dup2 := NewFolderManifest("dup-uid", "folderC")
		dup2Data, _ := json.Marshal(dup2)

		reader.On("Read", mock.Anything, "folderA/_folder.json", "").
			Return(&repository.FileInfo{Data: dataA}, nil)
		reader.On("Read", mock.Anything, "folderB/_folder.json", "").
			Return(&repository.FileInfo{Data: dup1Data}, nil)
		reader.On("Read", mock.Anything, "folderC/_folder.json", "").
			Return(&repository.FileInfo{Data: dup2Data}, nil)

		source := []repository.FileTreeEntry{
			{Path: "folderA/_folder.json", Blob: true},
			{Path: "folderB/_folder.json", Blob: true},
			{Path: "folderC/_folder.json", Blob: true},
		}
		...
		require.Len(t, issues, 1)
		assert.Equal(t, FolderMetadataIssueDuplicateUID, issues[0].Type)
		assert.Contains(t, issues[0].Message, "dup-uid")
	})
