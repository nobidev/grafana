...
// Uses a default reason (ConfigurationError) for unhealthy resources.
...
func buildReadyConditionFromHealth(healthStatus provisioning.HealthStatus) metav1.Condition {
	...
	return metav1.Condition{
		...
		Reason:  provisioning.ReasonConfigurationError,
		Message: message,
	}
}

// buildReadyConditionWithReason creates a Ready condition with a specific reason.
// This allows for more granular error classification (ConfigurationError vs ExternalError).
