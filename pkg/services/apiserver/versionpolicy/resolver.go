...
// any major, letting v2beta1 slip under a v1 cap). The per-group snapshot is kept only as the set of
// registered versions, used to validate config and reject writes at unknown versions.
type Resolver struct {
	order map[string][]string
}

// NewResolver captures the set of registered versions per group (the slice order is not significant).
