...
func (r *VersionPolicyRegistry) IsVersionAllowed(group, version string) (allowed bool, maxAllowedVersion string) {
	...
	// A cap is configured: a write at an empty or unregistered version cannot be ranked against it, so
	// fail closed rather than let an unknown version slip past the ceiling.
...
func (r *VersionPolicyRegistry) Validate() error {
	...
	// A typo'd group or version in boot config would otherwise be dropped silently, leaving the operator
	// believing a cap is in force when it is not. Reject unregistered boot values outright.
