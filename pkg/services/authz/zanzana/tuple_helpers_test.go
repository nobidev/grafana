...
func TestResourcePermissionTupleRegressionCases(t *testing.T) {
	tests := []struct {
		...
		{"wrong datasource resource errors", "loki.datasource.grafana.app", "queries", "Query", 0, "invalid datasource resource permission"},
		{"literal wildcard errors", "*.datasource.grafana.app", "datasources", "Query", 0, "invalid datasource resource permission"},
		{"empty prefix errors", ".datasource.grafana.app", "datasources", "Query", 0, "invalid datasource resource permission"},
		{"multi segment prefix errors", "foo.loki.datasource.grafana.app", "datasources", "Query", 0, "invalid datasource resource permission"},
		{"unrelated group remains singular", "loki", "datasources", "View", 1, ""},
		{"unknown dashboard verb errors", "dashboard.grafana.app", "dashboards", "Write", 0, "unknown resource permission verb"},
		{"unknown datasource verb errors", "loki.datasource.grafana.app", "datasources", "Write", 0, "unknown resource permission verb"},
	}
...
func requireGroupFilter(t *testing.T, tuple *openfgav1.TupleKey, groupResource string) {
	t.Helper()
	require.NotNil(t, tuple)
	require.NotNil(t, tuple.Condition)
	require.Equal(t, "group_filter", tuple.Condition.Name)
	require.Equal(t, groupResource, tuple.Condition.Context.Fields["group_resource"].GetStringValue())
}
