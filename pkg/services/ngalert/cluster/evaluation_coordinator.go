...
func (c *EvaluationCoordinator) Updates(ctx context.Context) <-chan bool {
	...
	go func() {
		...
		c.log.Info("This node is primary, starting alert rule evaluation")
		...
		for {
			...
			case <-ticker.C:
				...
				if next {
					c.log.Info("This node is now primary for alert rule evaluation")
				} else {
					c.log.Info("This node is now secondary, stopping alert rule evaluation")
				}
				current = next
				sendLatest(updates, current)
			}
