...
func TestIntegrationGetDescendantsPostorder(t *testing.T) {
	...
	t.Run("returns empty list for folder with no descendants", func(t *testing.T) {
		...
		require.NoError(t, err)
		defer folderStore.Delete(ctx, []string{parentFolder.UID}, orgID)
	...
	t.Run("returns single child in correct order", func(t *testing.T) {
		...
		require.NoError(t, err)
		defer folderStore.Delete(ctx, []string{childFolder.UID}, orgID)
	...
	t.Run("returns linear hierarchy in postorder", func(t *testing.T) {
		...
		defer folderStore.Delete(ctx, []string{child3.UID, child2.UID, child1.UID, parentFolder.UID}, orgID)
	...
	t.Run("returns branching hierarchy in postorder", func(t *testing.T) {
		...
		// Cleanup
		defer folderStore.Delete(ctx, []string{childA.UID, childB.UID, childC.UID, parentFolder.UID}, orgID)
	...
	t.Run("returns complex hierarchy in postorder", func(t *testing.T) {
		...
		// Cleanup in postorder
		defer folderStore.Delete(ctx, []string{childF.UID, childC.UID, childD.UID, childE.UID, childA.UID, childB.UID, parentFolder.UID}, orgID)
	...
	t.Run("handles deep nesting correctly", func(t *testing.T) {
		...
		defer func() {
			...
			for i := len(createdUIDs) - 1; i >= 0; i-- {
				folderStore.Delete(ctx, []string{createdUIDs[i]}, orgID)
			}
			folderStore.Delete(ctx, []string{parentFolder.UID}, orgID)
		}()
