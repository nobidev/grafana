...
func Initialize(ctx context.Context, cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	correlationAppInstaller, err := correlation.RegisterAppInstaller(cfg, featureToggles)
	if err != nil {
		return nil, err
	}
	v2 := appregistry.ProvideAppInstallers(featureToggles, playlistAppInstaller, pluginsAppInstaller, shortURLAppInstaller, correlationAppInstaller)
