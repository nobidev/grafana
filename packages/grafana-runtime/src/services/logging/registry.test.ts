...
describe('logging registry', () => {
  ...
  describe('initializeLoggersRegistry', () => {
    it('should populate the registry with loggers for all configured sources', () => {
      ...
      const pluginMetaLogger = getLogger('grafana.plugins.meta');
      const cachedPromiseLogger = getLogger('grafana.utils.getCachedPromise');
  ...
  describe('addLogger', () => {
    it('should add a functional logger', () => {
      ...
      addLogger('grafana.plugins.meta', defaults);

      const logger = getLogger('grafana.plugins.meta');
    ...
    it('should warn when adding the same logger twice', () => {
      addLogger('grafana.plugins.meta');
      addLogger('grafana.plugins.meta', { context: { env: 'dev' }, logToConsole: true });

      expect(warnSpy).toHaveBeenCalledWith(
        'LoggerRegistry: a logger with the source:grafana.plugins.meta already exists, keeping existing entry.'
      );
    });
  ...
  describe('getLogger', () => {
    ...
    describe.each([
      ...
      it('should warn and only throw in development environment', () => {
        if (throws) {
          expect(() => getLogger('grafana.plugins.meta')).toThrow(
            `LoggerRegistry: no logger 'grafana.plugins.meta' exists, are you calling getLogger before initializeLoggersRegistry function was called?`
          );
        }

        if (!throws) {
          getLogger('grafana.plugins.meta');
        }
  ...
  describe('setLogger', () => {
    it('should replace the registry entry with mocks', () => {
      setLogger('grafana.plugins.meta', {
        logDebug: jest.fn(),
        logError: jest.fn(),
        logInfo: jest.fn(),
...
function expectLoggerFunctions(
  ...
  expect(mockPushLog).toHaveBeenNthCalledWith(1, ['registry debug test'], {
    ...
    context: { ...defaults?.context, source: 'grafana.plugins.meta', pluginId: 'myorg-test-plugin' },
  });
  ...
  if (defaults?.logToConsole) {
    expect(spies.debugSpy).toHaveBeenCalledWith('registry debug test', {
      ...defaults.context,
      source: 'grafana.plugins.meta',
      pluginId: 'myorg-test-plugin',
    });
    expect(spies.errorSpy).toHaveBeenCalledWith(
      'registry error test',
      { ...defaults.context, source: 'grafana.plugins.meta', pluginId: 'myorg-test-plugin' },
