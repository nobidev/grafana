...
beforeEach(() => {
  mockLogger('grafana.plugins.meta');
});

it('should log a warning when something unexpected happens', () => {
  doSomethingThatWarns();

  expect(getLogger('grafana.plugins.meta').logWarning).toHaveBeenCalledWith(
...
it('should log an error on failure', () => {
  ...
  expect(getLogger('grafana.plugins.meta').logError).toHaveBeenCalledTimes(1);
  expect(getLogger('grafana.plugins.meta').logError).toHaveBeenCalledWith(expect.any(Error));
});
```
