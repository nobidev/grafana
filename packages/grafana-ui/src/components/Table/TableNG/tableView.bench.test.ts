import { cacheFieldDisplayNames, FieldType, getDisplayProcessor, createTheme, toDataFrame } from '@grafana/data';
import { type SortColumn } from '@grafana/react-data-grid';

import { encodeTableFilters, transformTableFilters, transformTableRows } from './TableViewContext';
...
benchmark.each([10_000, 100_000])('compares legacy and transformation view execution on %i rows', (length) => {
  ...
  const transforms = () =>
    transformTableRows(
      transformTableFilters(rows, frame.fields, encodeTableFilters(filters)).filteredRows,
      frame.fields,
      [],
      sort
    );
