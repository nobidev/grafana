...
import { tableFrameKey, filterByValueTransformer, sortByTransformer } from '@grafana/data/internal';
import { mockClientSize } from '@grafana/test-utils';

import { type AdHocTransformationsApi } from '../../PanelChrome/PanelContext';

import { TableNG } from './TableNG';
import { transformTableFilters, encodeTableFilters } from './TableViewContext';
...
it('computes each distribution using all other filters, independent of insertion order', () => {
  ...
  const result = transformTableFilters(
    rows,
    frame.fields,
    encodeTableFilters({
...
it('keeps standalone tables scoped to their supplied data even when it came from a dashboard view', async () => {
  ...
  const [data] = await lastValueFrom(
    transformDataFrame(
      encodeTableFilters(
        { value: { displayName: 'Value', filteredSet: new Set(), range: { min: 25, includeMissing: false } } },
        'utc',
        source,
        ''
      ).map((config) => ({ ...config, options: { ...config.options, target: undefined } })),
      [source]
    )
  );
