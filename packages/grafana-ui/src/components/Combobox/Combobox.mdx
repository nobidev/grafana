...
Options can be an array of objects with separate label and values, or an array of strings which will be used as both the label and value.
...
Each option object supports the following properties:
 - `value`: The value to return in the `onChange` handler
 - `label`: A human-friendly label to display in the dropdown
 - `description`: A short description of the option
 - `renderAccessory`: A function that returns a `ReactNode` to display to the right of the label. This function is called lazily only when the option is rendered for better performance. Note that only 16px will be reserved for this content. Larger accessories may cause text to clip.
 - `group`: The group to which the option belongs (see Grouping below)
